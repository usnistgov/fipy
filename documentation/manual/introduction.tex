...
\end{quote}
    ...
    Equation~\eqref{eqn:general-equation} or any linear combination of
    such terms.
...
\Class{Face} is defined by its bounding \Class{Vertex} objects. Although
one would think of Equation~\eqref{eqn:num:gen} conceptually being an
\Class{Equation} object consisting of \Class{Term} objects. In \FiPy{} this
is not the case as the \Class{Equation} object does not exist.
Instead Equation~\eqref{eqn:num:gen} is in fact represented by a
\Class{Term} object. \Class{Term} objects can be single entities such
as an \Class{ImplicitDiffusionTerm} or a linear combination of other
\Class{Term} objects that build up to form new term objects.
Different \Class{Term} objects, encapsulate the
building of different parts of the
\Class{SparseMatrix} that defines the solution of an equation.
\Class{BoundaryCondition} objects are used to
...
necessary to modify the \Class{SparseMatrix}. An equation constructed
from \Class{Term} objects can apply a unique \Class{Solver} to invert
its \Class{SparseMatrix} in the most expedient and stable fashion.  At any
