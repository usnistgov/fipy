...
# Should be true for all cases in fipy, but is not true in the 
# general case.
# Possible fix - 
# Adding both matrices into an empty matrix in the case where
# both start out filled.

# 2) addAt currently not guaranteed to work for fill-completed
# matrices, if elements are being added in new spots.
# Possible fix - create new matrix, insert into that matrix
# and then add the two together. Will be inefficient, but might
...
class _TrilinosMatrix(_SparseMatrix):
    ...
    # Addition is tricky. 
    # Trilinos interface is as such: A can be added into B, but A has to
    # be Filled() beforehand. If B is filled beforehand, this may or may not
    ...
    # With the latest changes to Trilinos (Add no longer FillComplete()s
    # the destination matrix) it is possible to make this work in the general
    # case by adding both matrices into an empty matrix. This has not yet
    # been implemented. It is not strictly necessary for FiPy currently, 
    # since all tests pass without it, but will be a good idea to have 
    # so that the matrix class can be used more generally without worrying
    # so much about the underlying trilinos.
        ...
        if self._getMatrix().Filled():
            if self._getMatrix().ReplaceGlobalValues(id1, id2, vector) != 0:
                ...
                warnings.warn("ReplaceGlobalValues returned error code in put", 
                               UserWarning, stacklevel=2)
                # Possible change to this part of the code - make a new matrix, 
                # use addAt to put the values in it, 
                # use replaceGlobalValues in the original matrix to zero out the terms 
                # that were not there, and add the old one into the new one, 
                # replace the old one
        ...
        if not self._getMatrix().Filled():
            ...
            if self._getMatrix().SumIntoGlobalValues(id1, id2, vector) != 0:
                ...
                warnings.warn("Summing into filled matrix returned error code",
                               UserWarning, stacklevel=2)
                # Possible change to this part of the code - make a new matrix, 
                # use addAt to put the values in it, at were not there, and add
                # the old one into the new one, replace the old one.
    ...
    # This function require take. This is a problem. 
