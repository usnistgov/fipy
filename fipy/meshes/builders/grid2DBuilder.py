...
class _Grid2DBuilder(_AbstractGridBuilder):
    ...
    if inline.doInline:
        ...
        def createCells(nx, ny, numFaces, numHorizFaces, numVertCols):
            ...
            """
            cellFaceIDs = numerix.zeros((4, nx * ny), _MeshMatrix.INDEX_TYPE)
        ...
        def createCells(nx, ny, numFaces, numHorizFaces, numVertCols):
            ...
            `f1` etc. refer to the faces
            """
            cellFaceIDs = numerix.zeros((4, nx * ny), dtype=_MeshMatrix.INDEX_TYPE)
            faceIDs = numerix.arange(numFaces, dtype=_MeshMatrix.INDEX_TYPE)
            if numFaces > 0:
