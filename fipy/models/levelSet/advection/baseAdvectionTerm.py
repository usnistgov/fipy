    ...
    def _buildMatrix(self, var, SparseMatrix, boundaryConditions=(), dt=None, equation=None, transientGeomCoeff=None, diffusionGeomCoeff=None):
        ...
        if var is self.var or self.var is None:
            ...
            if NCells > 0:
                ...
                minsq = numerix.sqrt(numerix.sum(numerix.minimum(differences.value, numerix.zeros((NCellFaces, NCells)))**2, axis=0))
                maxsq = numerix.sqrt(numerix.sum(numerix.maximum(differences.value, numerix.zeros((NCellFaces, NCells)))**2, axis=0))
