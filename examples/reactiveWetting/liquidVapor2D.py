...
>>> nx = 100
>>> dx = Lx / nx
>>> mesh = Grid1D(nx=nx, ny=nx, dx=dx, dy=dx)
...
>>> velocityXPrevious = velocity.copy()
>>> velocityYPrevious = velocity.copy()
...
...                + ConvectionTerm(coeff=[[1]] * density.getFaceValue() * velocityVector.getFaceValue(), var=velocityX) \
...                == DiffusionTerm(coeff=2 * viscosity, var=velocityX) \
...
...                + ConvectionTerm(coeff=[[1]] * density.getFaceValue() * velocity.getFaceValue(), var=velocityY) \
...                == DiffusionTerm(coeff=2 * viscosity, var=velocityY) \
...
>>> velocityY.constrain(0, mesh.getFacesLTop() & getFacesBottom())
...
>>> coupledEqn = massEq & momentumXEqn & momentumYEqn & potentialNCEqn
...
...         velocityVector[0] = veclocityX
...         velocityVector[1] = veclocityY
...
...         dt = min(dt, dx / max(abs(velocityVector.getMag())) * cfl)
...         
...         momentumXEqn.cacheMatrix()
...
...             break
...         else:
...             matrixDiagonal[:] = momentumXEqn.getMatrix().takeDiagonal()[:mesh.getNumberOfCells()]
