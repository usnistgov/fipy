...
The interface moves :math:`\\approx 3.4~\\unit{\\micro\\meter}` in
:math:`80~\\unit{\\milli\\second}`, driven by diffusion in the liquid
phase (compare the estimate above of :math:`0.1~\\unit{\\second}`).
For the next
:math:`20~\\unit{\\second}`, the interface stalls while the solute step
trapped in the solid phase diffuses outward
(:math:`(3.4~\\unit{\\micro\\meter})^2 / D_s =
\mathcal{O}(100~\\unit{\\second})`).  Once the solute gradient in the
...
:math:`3.2~\\unit{\\micro\\meter}` (recall the estimate of
