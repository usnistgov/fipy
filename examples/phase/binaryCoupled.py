...
The interface moves :math:`\\approx \\qty{2.8}{\\micro\\meter}` in
:math:`\\qty{70}{\\milli\\second}`, driven by diffusion in the liquid
phase (compare the estimate above of :math:`\\qty{0.1}{\\second}`).
For the next
:math:`\\qty{12}{\\second}`, the interface stalls while the solute step
trapped in the solid phase diffuses outward
(:math:`(\\qty{2.8}{\\micro\\meter})^2 / D_s =
\mathcal{O}(\\qty{80}{\\second})`).  Once the solute gradient in the
...
:math:`\\qty{2.9}{\\micro\\meter}` (recall the estimate of
