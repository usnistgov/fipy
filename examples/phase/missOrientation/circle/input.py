...
## 
 ...
 #                                last update: 9/3/04 {10:28:57 PM} 
...
.. raw:: latex
    ...
    $$ \\phi = 1 \;\; \\forall x $$
    $$ \\theta = 1 \;\; \\text{for} \;\; (x - L / 2)^2 + (y - L / 2)^2 > (L / 4)^2 $$
    $$ \\theta = 0 \;\; \\text{for} \;\; (x - L / 2)^2 + (y - L / 2)^2 \le (L / 4)^2 $$
    $$ T = 1 \;\; \\forall x $$
...
data and compares it with the `theta` variable.
   ...
   >>> filestream=os.popen('gunzip --fast -c < %s/%s'%(examples.phase.missOrientation.circle.__path__[0],'test.gz'),'r')
   >>> import cPickle
