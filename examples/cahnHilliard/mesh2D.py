...
if __name__ == "__main__":
    parser = OptionParser()
    parser.set_defaults(N=1000000)
    parser.set_defaults(disp="disp")
    parser.set_defaults(runtime="runtime")
    parser.add_option("-n","--numberOfElements", type = "int", dest = "N")
    parser.add_option("-d","--no-display", dest = "disp", \
                          help = "Input 'nodisp' to override display module") 
    parser.add_option("-m", "--measureTime", dest = "time")
    (options, args) = parser.parse_args()
    nx = ny = int(numerix.sqrt(options.N))
...
#time_mesh_and_phi = time.time() - t0
#print "time to make mesh and phi", time_mesh_and_phi
...
if __name__ == "__main__":
    if options.disp == "disp":
        viewer = Viewer(vars=(phi,), datamin=0., datamax=1.)
        viewer.plot()
        raw_input()
    if options.time == "runtime":
        runtime = time.time()-t0
        print 'runtime:', str(runtime).rjust(15)
