...
the :class:`ExponentialConvectionTerm` and :class:`PowerLawConvectionTerm` will both
...
Although no error was generated by the use of :class:`Grid2D`, :term:`FiPy` 1.0 supports
...
The :class:`Grid2D` class with `ny = 1` still works perfectly well for 1D
problems, but the :class:`Grid1D` class is slightly more efficient, and it makes
...
The :class:`FixedValue` boundary conditions at the left and right are unchanged,
...
The biggest change between :term:`FiPy` 0.1 and :term:`FiPy` 1.0 is that :class:`Equation`
objects no longer exist at all.  Instead, :class:`Term` objects can be simply
...
automatically was that a :class:`ConvectionTerm` must know about any
:class:`DiffusionTerm` in the equation in order to calculate a Peclet number.
Now, the :class:`DiffusionTerm` must be explicitly passed to the :class:`ConvectionTerm`
...
`solver` if desired, although the default :class:`LinearPCGSolver` is usually
suitable):
...
.. note:: 
   In version 0.1, the :class:`Equation` object had to be told about the
   :class:`Variable`, :class:`Solver`, and :class:`BoundaryCondition` objects
   when it was created (and it, in turn, passed much of this information to the
   :class:`Term` objects in order to create them). In version 1.0, the
   :class:`Term` objects (and the equation assembled from them) are abstract.
   The :class:`Variable`, :class:`Solver`, and :class:`BoundaryCondition`
...
:class:`Viewer` classes have been moved and renamed.  This error could be resolved
...
Instead, rather than instantiating a particular :class:`Viewer` (which you can
still do, if you desire), a generic "factory" method will return a :class:`Viewer`
