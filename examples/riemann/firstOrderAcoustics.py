...
for step in range(timesteps):
    ...
    eqn.solve(q, dt=dt)
    setBCs(q)
    numerix.savetxt('data' + str(step + 1) + '.txt', q.value)
    elapsedTime += dt
...
filepath = '/users/wd15/work/matforge/wd15/clawpack-4.6.1/apps/acoustics/1d/example2/_output/fort.q0001'
#claw = 
#print max(abs(q[0] - 
claw =  numerix.loadtxt(filepath, skiprows=6).swapaxes(0,1)
print max(abs(q[0,2:-2] - claw)[0])
print max(abs(q[1,2:-2] - claw)[1])
...
##import os 
numerix.savetxt('dataLast.txt', q.value)
##filepath = os.path.splitext(__file__)[0] + 'ClawpackFirstOrder.gz'
filepath = '/users/wd15/work/matforge/wd15/clawpack-4.6.1/apps/acoustics/1d/example2/_output/fort.q0007'
claw =  numerix.loadtxt(filepath, skiprows=6).swapaxes(0,1)
print max(abs(q[0,2:-2] - claw)[0])
print max(abs(q[1,2:-2] - claw)[1])
print q[:,2:-2].allclose(numerix.loadtxt(filepath, skiprows=6).swapaxes(0,1))
